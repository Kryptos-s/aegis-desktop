...
public class FreeOtpImporter extends DatabaseImporter {
    /** GCM tag length in bytes, used when the serialized parameters do not state one. */
    ...
    public static class EncryptedState extends State {
        ...
        public State decrypt(char[] password) throws DatabaseImporterException {
            PBKDF.Params params = new PBKDF.Params(_mkAlgo, MASTER_KEY_SIZE, password, _mkSalt, _mkIterations);
            SecretKey passKey = PBKDF.deriveKey(params);
        ...
        public State decrypt(SecretKey passKey) throws DatabaseImporterException {
            byte[] masterKeyBytes;
            try {
                GCMParameterSpec spec = parseGcmSpec(_mkParameters);
    ...
    public static class DecryptedStateV2 extends DatabaseImporter.State {
        ...
        private VaultEntry convertEntry(JSONObject encObj, JSONObject tokenObj)
            ...
            try {
                ...
                GCMParameterSpec spec = parseGcmSpec(parameters);
    ...
    /**
     * The Android app passes an IvParameterSpec and lets the provider pick the tag length, which
     * Conscrypt allows but SunJCE does not, so the length is read out of the ASN.1 instead.
     */
    private static GCMParameterSpec parseGcmSpec(byte[] parameters) throws IOException {
        byte[] nonce = parseNonce(parameters);
        return new GCMParameterSpec(parseTagLength(parameters) * Byte.SIZE, nonce);
    }

    /** Returns the tag length in bytes, taken from the ASN.1 GCMParameters if it states one. */
    private static int parseTagLength(byte[] parameters) throws IOException {
        ...
        if (prim instanceof ASN1Sequence) {
            for (ASN1Encodable enc : (ASN1Sequence) prim) {
                if (enc instanceof ASN1Integer) {
                    int tagLen = ((ASN1Integer) enc).intValueExact();
                    if (tagLen < 12 || tagLen > 16) {
                        throw new IOException(String.format("Bad GCM tag length: %d", tagLen));
                    }
                    return tagLen;
                }
            }
