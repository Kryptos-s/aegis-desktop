...
public class VaultFileCredentials implements Serializable {
    ...
    /**
     * Destroys the master key held by these credentials. Called when the vault is locked, so that
     * the key stops being reachable as early as the runtime permits.
     */
    public void destroy() {
        if (_key != null) {
            _key.destroy();
